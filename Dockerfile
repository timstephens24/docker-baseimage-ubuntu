...
# grab base tarball
RUN echo "**** installing base ubuntu ****" \
  && mkdir /root-out \
  && curl -o /rootfs.tar.gz -L \
...
RUN echo "**** Ripped from Ubuntu Docker Logic ****" \
  ...
  && apt-get install -y --no-install-recommends at beignet-opencl-icd ca-certificates curl gnupg i965-va-driver intel-media-va-driver-non-free jq libfontconfig1 libfreetype6 libssl1.1 mesa-va-drivers ocl-icd-libopencl1 tzdata udev unrar \
